    ...
    def _NH_SIPSubscriptionDidFail(self, notification):
        ...
        if self.stopping or notification.data.code in (401, 403, 407) or self.success:
            ...
            if not self._subscription_routes or time() > self._subscription_timeout:
                ...
                self.subscription = Subscription(FromHeader(self.account.uri, self.account.display_name), self.target, ContactHeader(self.account.contact[route.transport]), "xcap-diff", route, credentials=self.account.credentials, refresh=self.account.presence.subscribe_interval)
