...
static void SHA1_Transform(uint32_t* state, const uint8_t* input)
{
...
void SHA1_Final(SHA1_CTX* context, uint8_t digest[SHA1_DIGEST_SIZE])
{
    ...
    ((uint8_t *) context->block)[i++] = 0x80;
    if (i > SHA1_BLOCK_SIZE - 8) {
        memset(((uint8_t *) context->block) + i, 0, SHA1_BLOCK_SIZE - i);
        SHA1_Transform(context->state, context->block);
        memset((uint8_t *) context->block, 0, SHA1_BLOCK_SIZE - 8);
    } else {
        memset(((uint8_t *) context->block) + i, 0, SHA1_BLOCK_SIZE - 8 - i);
    }
    ...
    context->block[63] = (bit_count >>  0) & 0xff;
    SHA1_Transform(context->state, context->block);

    digest[ 0] = (uint8_t) ((context->state[0] >> 24) & 0xff);
    digest[ 1] = (uint8_t) ((context->state[0] >> 16) & 0xff);
    digest[ 2] = (uint8_t) ((context->state[0] >>  8) & 0xff);
    digest[ 3] = (uint8_t) ((context->state[0]      ) & 0xff);
    digest[ 4] = (uint8_t) ((context->state[1] >> 24) & 0xff);
    digest[ 5] = (uint8_t) ((context->state[1] >> 16) & 0xff);
    digest[ 6] = (uint8_t) ((context->state[1] >>  8) & 0xff);
    digest[ 7] = (uint8_t) ((context->state[1]      ) & 0xff);
    digest[ 8] = (uint8_t) ((context->state[2] >> 24) & 0xff);
    digest[ 9] = (uint8_t) ((context->state[2] >> 16) & 0xff);
    digest[10] = (uint8_t) ((context->state[2] >>  8) & 0xff);
    digest[11] = (uint8_t) ((context->state[2]      ) & 0xff);
    digest[12] = (uint8_t) ((context->state[3] >> 24) & 0xff);
    digest[13] = (uint8_t) ((context->state[3] >> 16) & 0xff);
    digest[14] = (uint8_t) ((context->state[3] >>  8) & 0xff);
    digest[15] = (uint8_t) ((context->state[3]      ) & 0xff);
    digest[16] = (uint8_t) ((context->state[4] >> 24) & 0xff);
    digest[17] = (uint8_t) ((context->state[4] >> 16) & 0xff);
    digest[18] = (uint8_t) ((context->state[4] >>  8) & 0xff);
    digest[19] = (uint8_t) ((context->state[4]      ) & 0xff);
}
