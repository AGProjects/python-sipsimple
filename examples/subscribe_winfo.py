...
from pypjua import *
from pypjua.applications import watcherinfo_xml

queue = Queue()
packet_count = 0
start_time = None
winfo = watcherinfo_xml.WatcherInfo()
...
def parse_options():
    retval = {}
    description = "This example script will use the specified SIP account to subscribe to the presence state of the specified presentity. If presentity is not specified, it will default to own presentity. The program will unsubscribe and quit when CTRL+D is pressed."
    ...
    parser.add_option("-v", "--event", type="string", dest="event", default="presence", help='Event whose watcher info to subscribe to. Default is "presence".')
    ...
    try:
        ...
        try:
            username_domain, retval["password"], presentity = args
        except ValueError:
            username_domain, retval["password"] = args
            presentity = username_domain
