...
def event_handler(event_name, **kwargs):
    ...
    if event_name == "publish_state":
        ...
        print 'Received NOTIFY for "presence.winfo" with Conent-Type %(content_type)s/%(content_subtype)s":\n%(body)s' % kwargs
...
def do_publish(username, domain, password, proxy_ip, proxy_port, expires):
    e = Engine(event_handler, auto_sound=False, do_sip_trace=False)
    e.start()
    try:
        if proxy_ip is None:
            route = None
...
def parse_options():
    retval = {}
    description = "This example script will publish the presence state of the specified SIP account as one of a set of predefined values, based on user input. Additionally it will subscribe to presence.winfo and display the contents of the NOTIFY."
