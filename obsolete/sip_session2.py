...
class ChatManager(NotificationHandler):
    ...
    def _handle_incoming(self, session, data):
        ...
        if self.auto_answer:
            ...
            #    replies += list('aAcC')
            #    replies_txt += '/a/c'
            question = 'Incoming %s request from %s, do you accept? (%s) ' % (session._chatsession.format_stream_info(), format_identity(session.inv.from_header), replies_txt)
            with linked_notification(name='SIPSessionChangedState', sender=session) as q:
...
def start(options, console):
    ...
    try:
        ...
        if hasattr(options.account, "ice") and len(options.account.ice.stun_servers) > 0:
